...
for cpu in "${cpus[@]}"; do
    ...
    ssh $OW_SERVER_NODE "docker build --build-arg GOGC=1 --build-arg GOMEMLIMIT=128MiB --build-arg MAXPROCS=${cpu} -t go-server-image /users/am_CU/openwhisk-devtools/docker-compose/Native/Go/"
