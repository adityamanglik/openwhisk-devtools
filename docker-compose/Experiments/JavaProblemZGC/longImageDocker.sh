...
for GC in "${GC_COLLECTORS[@]}"; do
  for MEM_SIZE in "${MEMORY_SIZES[@]}"; do
    ...
    curl "http://node0:8601/ImageProcess?seed=999&arraysize=99&requestnumber=567" >> Results/"img_'$GC'_'$MEM_SIZE'.txt"
    ...
    echo "" >> Results/"img_'$GC'_'$MEM_SIZE'.txt"

    # Run the analysis script
    python analysis.py >> Results/"img_'$GC'_'$MEM_SIZE'.txt"

    # Optional: collect and store results for each GC and memory size run
    mv locust_stats_history.csv Results/img_${GC}_${MEM_SIZE}.csv
